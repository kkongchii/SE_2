...
void Product::changeAvgPurchaseSatisfaction(int inputValue)
{
	...
	int count = this->satisfactionCount;

	this->avgPurchaseSatisfaction = (satisfaction * count + inputValue) / (count + 1);
...
int Product::getMyPurchaseSatisfaction()
{
	return this->myPurchaseSatisfaction;
}