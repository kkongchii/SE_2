...
#include "JobApplicationManagement/ViewJobApplications.h"
#include "JobApplicationManagement/CancelJobApplication.h"
    ...
    while (!is_program_exit) {
        ...
        case 4: {
            ...
            case 3: {
                ...
                cout << "4.3. 지원 정보 조회\n"; // 출력 양식
                if(isLogin == 2) {
                    ViewJobApplications controlViewJobApplication = ViewJobApplications();

                    vector<User*> userList = userDB.getUserList();
                    string userId = currentLoginUser->getId();
                    GeneralUser* currentGeneralUser;

                    for(auto it = userList.begin(); it!=userList.end(); it++) {
                        if((*it)->getId() == userId){
                            currentGeneralUser = (GeneralUser*) *it;
                            break;
                        }
                    }

                    controlViewJobApplication.getUI()->showJobApplication(currentGeneralUser);
                } else {    // 일반 유저로 로그인되어 있지 않다면 예외처리
                    cout << "This feature is only available to General user.\n" << endl;
                }
                break;
            }
            case 4: {
                // 4.4 지원 취소
                cout << "4.4. 지원 취소\n";
                int companyRegistrationNum;
                fscanf(in_fp, "%s", &companyRegistrationNum);

                CancelJobApplication controlCancelJobApplication = CancelJobApplication();
                ...
                controlCancelJobApplication.getUI()->selectJobApplication(currentGeneralUser, companyRegistrationNum);
            }
            }
            break;
        }
